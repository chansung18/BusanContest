...
class AirQualityViewController: UIViewController, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        for gps in airQualityLocationData{
            
          let zoneNumber : Int = groudLocationKey[gps]!
            airQulityParser.beginParsing(zoneNumber)
            centerMapOnLocation(gps.latitude , longitude: gps.longitude , localName: airQulityParser.dataSet[i].site)
            
            i++
            
        }
        
        print("datazone ->>\(airQulityParser.dataSet)")
